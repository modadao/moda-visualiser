...
void main() {
  ...
  float springY = (springTextureIndex / 20.);
  float noiseYModifier = cubicOut(springY);
  float noiseY = (springY * noiseYModifier) * 2. - 1.;
  vec4 c = texture2D(u_springTexture, vec2((progress / PI) + u_time * 0.02, 0.5));
  float a = (0.5 + c.g);
  ...
  float znoise = (noise(vec3(noiseCoords.z - noiseY + rotation, noiseCoords.x + noiseY, noiseCoords.y + a * u_noiseRamp)) - 0.5) * noiseY * u_noiseScale * a;
